...
using PubsuiteProductRefreshStaticTestLib.Testcases;
...
{
    public class PubsuiteProductRefreshStaticTestLib
    {
        /// <summary>
        /// Run WUSAFX test
        /// </summary>
        ...
        {
            TPubsuiteProductRefreshStaticTest pubsuiteStaticTest = new TPubsuiteProductRefreshStaticTest();
            using (var dbContext = new PatchTestDataClassDataContext())
            {
                pubsuiteStaticTest.Title = inputdata.Title;
                pubsuiteStaticTest.CaseID = dbContext.TPubsuiteProductRefreshTestCaseInfos.Where(item => item.Name.Contains(caseName)).Select(item => item.ID).Single();
                ...
                dbContext.TPubsuiteProductRefreshStaticTests.InsertOnSubmit(pubsuiteStaticTest);
        ...
        {
            TPubsuiteProductRefreshStaticResultDetail pubsuiteStaticResultDetail = new TPubsuiteProductRefreshStaticResultDetail();
            using (var dbContext = new PatchTestDataClassDataContext())
            {
                ...
                var CaseID = dbContext.TPubsuiteProductRefreshTestCaseInfos.Where(item => item.Name.Contains(caseName)).Select(item => item.ID).Single();
                var TestID = dbContext.TPubsuiteProductRefreshStaticTests.Where(item => item.CaseID.Equals(CaseID) && item.UpdateGUID.Equals(inputdata.UpdateID)).Select(item => item.ID).Single();
                pubsuiteStaticResultDetail.TestID = TestID;
                dbContext.TPubsuiteProductRefreshStaticResultDetails.InsertOnSubmit(pubsuiteStaticResultDetail);
                dbContext.SubmitChanges();
            }
        }
    }
